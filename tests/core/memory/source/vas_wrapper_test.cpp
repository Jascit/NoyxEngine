...
NOYX_TEST(vas_reserve_memory, invalid_args_test) {
  ...
  NOYX_ASSERT_TRUE_MESSAGE(resp.err == Error::INVALID_ARG, "reserve_memory should fail with size 0");

  // Test 2: LARGE_PAGES flag in reserve (must be committed immediately, so reserve fails)
  ...
  NOYX_ASSERT_TRUE_MESSAGE(resp.err == Error::INVALID_ARG, "reserve_memory should fail with LARGE_PAGES flag");

  // Test 3: NON_PAGED flag in reserve
  req.alloc_flags = static_cast<std::uint32_t>(Flag::NON_PAGED);
  resp = reserve_memory(req, allocation_granularity);
  NOYX_ASSERT_TRUE_MESSAGE(resp.err == Error::INVALID_ARG, "reserve_memory should fail with NON_PAGED flag");

  // Test 4: Invalid alignment (less than allocation_granularity)
  req.alloc_flags = static_cast<std::uint32_t>(Flag::NONE);
  req.alignment = allocation_granularity / 2;
  resp = reserve_memory(req, allocation_granularity);
  NOYX_ASSERT_TRUE_MESSAGE(resp.err == Error::INVALID_ARG, "reserve_memory should fail if alignment < granularity");
  ...
  resp = reserve_memory(req, allocation_granularity);
  NOYX_ASSERT_TRUE_MESSAGE(resp.err == Error::INVALID_ARG, "reserve_memory should fail with unaligned preferred_addr");
}
...
NOYX_TEST(vas_commit_decommit_memory_test, invalid_args_test) {
  ...
  commit_req.size = 0;
  CommitResponse commit_resp = commit_pages(commit_req, page_size);
  NOYX_ASSERT_TRUE_MESSAGE(commit_resp.err == Error::INVALID_ARG, "commit_pages should fail with size 0");
  ...
  NOYX_ASSERT_TRUE_MESSAGE(commit_resp.err == Error::INVALID_ARG, "commit_pages should fail if LARGE_PAGES is used with non-null base");
  ...
  DecommitResponse decommit_resp = decommit_pages(decommit_req);
  NOYX_ASSERT_TRUE_MESSAGE(decommit_resp.err == Error::INVALID_ARG, "decommit_pages should fail with null base");
  ...
  NOYX_ASSERT_TRUE_MESSAGE(decommit_resp.err == Error::INVALID_ARG, "decommit_pages should fail with size 0");

  // Test 5: Decommit with LARGE_PAGES flag
  decommit_req.size = page_size;
  decommit_req.alloc_flags = static_cast<std::uint32_t>(Flag::LARGE_PAGES);
  decommit_resp = decommit_pages(decommit_req);
  NOYX_ASSERT_TRUE_MESSAGE(decommit_resp.err == Error::INVALID_ARG, "decommit_pages should fail with LARGE_PAGES flag");
}