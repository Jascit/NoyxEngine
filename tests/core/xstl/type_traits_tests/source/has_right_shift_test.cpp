...
constexpr void tt_has_right_shift_test_value() {
  constexpr bool actual = xstl::has_right_shift<T, U>::value;
#if TEST_WITH_STATIC_ASSERT
  NOYX_ASSERT_TRUE_MESSAGE(
    actual == Expected,
    "has_right_shift<" #T ", " #U "> returned incorrect value"
  );
#else
  NOYX_ASSERT_TRUE_MESSAGE(
    actual == Expected,
...
struct TestTypeInvokerHasRightShift {
  void operator()() const {
    ...
    tt_has_right_shift_test_value<int, int, true>();
    tt_has_right_shift_test_value<unsigned, int, true>();
    tt_has_right_shift_test_value<int, unsigned, true>();
    tt_has_right_shift_test_value<const int, int, true>();
...
NOYX_TEST(HasRightShift, UnitTest) {
  TestTypeInvokerHasRightShift{}();
}
