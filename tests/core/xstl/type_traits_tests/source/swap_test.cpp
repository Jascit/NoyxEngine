...
struct TestTypeInvokerIsSwappable
{
    constexpr void operator()() const
    {
        using U = T;
        // для базових типів
        tt_is_swappable_test_type<             U, true>();  // int: movable + assignable
        tt_is_swappable_test_type<const        U, false>();  // const int: не можна присвоїти
        tt_is_swappable_test_type<      U&, true>();  // int&: теж movable + assignable
        tt_is_swappable_test_type<      U&&, true>();
        // масив — не move_assignable
        tt_is_swappable_test_type<U[3], false>();
        // вказівник
        tt_is_swappable_test_type<U*, true>();
    }
};
...
{
    // Перевіримо на наборі типів: int, long, char, double
    xstl_test_detail::for_each_type<
        std::tuple<int, long, char, double>,
        TestTypeInvokerIsSwappable
    >();
}
...
{
    // Перевіряємо на вбудованих типах через структуру-обгортку
    ...
    // Перевіряємо на нашому movable-only типі
