...
NOYX_TEST(system_info, smoke_test)
{
    ...
    std::cout << std::endl
              << "allocation_granularity: " << SystemInfo::allocation_granularity << std::endl;
    ...
    auto end                                    = start + std::chrono::milliseconds(10000);
    while (end >= start)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(5000));
        std::chrono::steady_clock::time_point check = std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::milliseconds>(check - start).count() >= 5000)
        {
