...
FORCE_INLINE static DWORD to_windows_flags(std::uint32_t flags) noexcept {
  DWORD result = 0;
  if (flags & Flag::LARGE_PAGES) result |= MEM_LARGE_PAGES;
  if (flags & Flag::PAGE_64KB) result |= MEM_64K_PAGES;
  return result;
}

static ReserveResponse reserve_memory_windows(std::uint64_t size, void* preferred_addr, DWORD alloc_flags) noexcept {
...
static CommitResponse commit_pages_windows(std::uint64_t size, void* addr, DWORD alloc_flags, DWORD prots,
...
FORCE_INLINE int to_unix_flags(std::uint32_t flags) noexcept {
  ...
  if (flags & Flag::FIXED_ADDRESS) result |= MAP_FIXED;
#if defined (NOYX_LINUX)
if (flags &Flag::LARGE_PAGES) result|= MAP_HUGETLB;
#endif
return result;
}
...
[[nodiscard]] ReserveResponse noyxcore::memory::vas::reserve_memory(const ReserveRequest& req,
  ...
  if (req.alloc_flags & Flag::LARGE_PAGES) return {nullptr, 0, Error::INVALID_ARG};
  if (req.alloc_flags & Flag::NON_PAGED) return {nullptr, 0, Error::INVALID_ARG};
  if (req.size == 0) return {nullptr, 0, Error::INVALID_ARG};
  if (req.alloc_flags & Flag::FIXED_ADDRESS && req.preferred_addr) return {nullptr, 0, Error::INVALID_ARG};
  ...
  if (alignment < allocation_granularity) return {nullptr, 0, Error::INVALID_ARG};
  if (alignment % allocation_granularity != 0) return {nullptr, 0, Error::INVALID_ARG};
...
[[nodiscard]] DecommitResponse noyxcore::memory::vas::decommit_pages(const DecommitRequest& req) noexcept {
  if (req.base == nullptr) return {Error::INVALID_ARG};
  if (req.size == 0) return {Error::INVALID_ARG};
  if (req.alloc_flags & Flag::LARGE_PAGES) return {Error::INVALID_ARG};
