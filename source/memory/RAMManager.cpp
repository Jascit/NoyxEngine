...
RAMManager::RAMRegion::PageBitmap::PageBitmap(std::size_t granularity,
    ...
    : m_granularity(granularity), m_bitmap((total_size/granularity)) {}

bool RAMManager::RAMRegion::PageBitmap::is_range_free(
    void* base, void* ptr, std::size_t size) const noexcept {
  ...
  std::size_t left_bits =
      (size + m_granularity - 1) / m_granularity;  // ceil-div
  ...
  while (remaining > 0) {
    ...
    std::size_t bits_to_scan =
        std::min<std::size_t>(64 - local_offset, remaining);

    std::uint64_t mask =
        (bits_to_scan == 64) ? ~0ULL : ((1ULL << bits_to_scan) - 1ULL);
...
void RAMManager::RAMRegion::PageBitmap::set_range(void* base, void* ptr,
                                                  std::size_t size,
                                                  bool value) noexcept {
  ...
  while (left_bits > 0) {
    ...
    std::size_t bits_to_change =
        std::min<std::size_t>(64 - local_offset, left_bits);
