...
#include <stdint.h>
  ...
  typedef struct { vaw_err_t err; } vaw_release_resp_t;
  ...
  /* Map / Unmap */
  typedef struct { void* base; uint64_t offset; uint64_t size; uint32_t prot; } vaw_map_req_t;
  typedef struct { vaw_err_t err; uint64_t ptr; } vaw_map_resp_t;
  vaw_map_resp_t vaw_map(const vaw_map_req_t* req);

  typedef struct { void* ptr; uint64_t size; } vaw_unmap_req_t;
  typedef struct { vaw_err_t err; } vaw_unmap_resp_t;
  vaw_unmap_resp_t vaw_unmap(const vaw_unmap_req_t* req);

  /* Advise */
  typedef struct { void* base; uint64_t offset; uint64_t size; int advice; } vaw_advise_req_t;
  typedef struct { vaw_err_t err; } vaw_advise_resp_t;
