...
void get_memory_info(uint64_t &used_memory, uint64_t &available_memory, uint64_t &total_swap,
                     uint64_t &free_swap) noexcept;

void get_system_info(uint32_t &allocation_granularity, uint32_t &page_size,
...
public:
    ...
    FORCE_INLINE static void update_cpu_usage() noexcept { details::get_cpu_usage(cpu_usage); };
