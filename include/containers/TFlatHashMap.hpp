...
public:
    ...
    constexpr void
    construct(Allocator &alloc, slot_type *slot,
              Args &&...args) noexcept(std::is_nothrow_constructible_v<value_type, Args...>)
    {
        using target_type =
            std::conditional_t<bLayoutCompatible, typename slot_type::mutable_value_type,
                               typename slot_type::value_type>;
    ...
    constexpr void destroy(Allocator &alloc,
                           slot_type *slot) noexcept(std::is_nothrow_destructible_v<value_type>)
    {
        using target_type =
            std::conditional_t<bLayoutCompatible, typename slot_type::mutable_value_type,
                               typename slot_type::value_type>;
...
private:
    FORCE_INLINE constexpr void emplace(slot_type *slot) noexcept { new (slot) slot_type; }
};

// Count should be 2^k(8/16/32/64 Byte, aligned to SIMD) & > 8
template <typename Policy, size_t Count = 8,
          typename Allocator = std::allocator<typename Policy::slot_type::value_type>>
class SwissTable
{
public:
...
public:
    ...
    constexpr inline void next() noexcept { current_ = (current_ + step_) % capacity_; };

    [[nodiscard]] inline constexpr size_t current() const noexcept { return current_; }
...
public:
    ...
    [[nodiscard]] constexpr FORCE_INLINE bool need_grow() const noexcept { return size_ < ; };
