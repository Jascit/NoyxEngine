...
inline std::uint64_t bswap64_u64(std::uint64_t x)
{
    return ((x & 0xFFull) << 56) | ((x & 0xFF00ull) << 40) | ((x & 0xFF0000ull) << 24) |
           ((x & 0xFF000000ull) << 8) | ((x >> 8) & 0xFF000000ull) | ((x >> 24) & 0xFF0000ull) |
...
// For clarity: implement minimal round/merge helpers that mirror xxh64
// arithmetic
...
// These compute something equivalent to the corresponding tail processing in
// xxh64. seed parameter left so you can pass non-zero if needed.
...
[[nodiscard]] inline hash64_t XXH64(const void *bytes_array, uint64_t length,
