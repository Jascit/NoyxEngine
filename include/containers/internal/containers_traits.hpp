...
constexpr bool can_use_memset_v =
    std::conjunction_v<is_contiguous_iterator<It>, std::is_scalar<iter_val_t<It>>,
...
constexpr bool can_use_memcpy_v =
    std::conjunction_v<is_contiguous_iterator<It>, std::is_trivially_copyable<iter_val_t<It>>,
                       std::negation<std::is_volatile<std::remove_reference_t<iter_ref_t<It>>>>>;
...
concept PairLike = requires {
    typename pair_traits<T>::first_type;
    typename pair_traits<T>::second_type;
};
...
private:
    ...
    static constexpr bool layout_compatible_v =
        sizeof(P) == sizeof(Raw) && has_first_second_offsets_v<P> &&
        has_first_second_offsets_v<Raw> && (alignof(P) == alignof(Raw)) &&
...
public:
    static constexpr bool value =
        std::is_standard_layout_v<key_type> && std::is_standard_layout_v<RawPair> &&
        (offsetof(RawPair, first) == 0) && layout_compatible_v<std::pair<key_type, T>, RawPair>() &&
